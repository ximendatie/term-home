...
- 看见当前在运行什么
- 看见任务何时完成、失败或中断
- 点击后快速查看 title 和 detail

### 工具无关
这个项目不应该只服务某一个 CLI 或某一个模型供应商。

### 人在回路中
Agent 可以自动执行，但顶部状态层首先要做到稳定监测与清晰呈现。
...
- `Sources/TermHomeNative/`：原生 macOS 壳层脚手架（顶部胶囊窗口 + 展开面板 + 任务详情）。
...
第一版只做最小闭环，不做桌面工具箱，也不做反馈式动作：

- 原生 macOS 常驻 app。
- 顶部入口采用顶部胶囊 / notch 风格收起态。
- 点击后展开一个简单面板。
- 只展示当前 1 个活动任务。
- 只关注基础状态：`running` / `completed` / `failed` / `cancelled`。
- 展开态只展示 `title` 与 `detail` 这两层关键信息。
